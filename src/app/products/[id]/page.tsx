...
function getCategoryId(category: unknown): string {
  ...
  if (typeof category === "object") {
    ...
    if (categoryObject._id) {
      return String(categoryObject._id).trim();
    }

    if (categoryObject.$oid) {
      return String(categoryObject.$oid).trim();
    }
  }
...
export default async function ProductDetailsPage({
  ...
  const product: Product = productData.product || productData.data;
  ...
  const matchedCategory = categories.find(
    (category) => String(category._id).trim() === productCategoryId,
  );
      ...
      <main className=" container mx-auto max-w-7xl px-4 py-6">
        ...
        <div className="mt-8 grid gap-10 md:grid-cols-2">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-sm font-medium text-blue-600">{categoryName}</p>
